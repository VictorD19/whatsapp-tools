...
export class EvolutionAdapter implements IWhatsAppProvider {
  ...
  async createInstance(config: CreateInstanceDto): Promise<InstanceResult> {
    ...
    const createdInstanceName = res.instance.instanceName

    // Garante que syncFullHistory esteja sempre ativo
    await this.http.post(`/settings/set/${createdInstanceName}`, {
      rejectCall: false,
      msgCall: '',
      groupsIgnore: false,
      alwaysOnline: false,
    ...
    return {
      instanceId: createdInstanceName,
      status,
    }
  }
  ...
  async sendAudio(
    ...
    const audioValue = payload.base64 ?? payload.url
